...
Deep QROM (DQROM) implementation.


Key classes:
- RewardMachine: simple automaton API
- OpponentModel: MLP predicting opponents' joint-action id
- QNetwork: small MLP that maps (obs, op_action_onehot) -> n_actions (used per RM state)
- ReplayBuffer
- DQROMAgent: agent performing counterfactual updates per RM state
...
class QNetwork(nn.Module):
    def __init__(self, obs_dim: int, op_action_dim: int, n_actions: int, hidden_sizes=[512, 512, 512]):
        super().__init__()
        in_dim = obs_dim + op_action_dim
        layers = []
        for h in hidden_sizes:
            layers.append(nn.Linear(in_dim, h))
            layers.append(nn.ReLU())
    ...
    def forward(self, obs: torch.Tensor, op_action_onehot: torch.Tensor):
        x = torch.cat([obs, op_action_onehot], dim=-1)
        return self.net(x)
...
class DQROMAgent:
    def __init__(self,
        ...
        self.q_nets = nn.ModuleList([QNetwork(obs_dim, n_op_actions, n_actions).to(device) for _ in range(self.n_rm_states)])
        self.q_targets = nn.ModuleList([QNetwork(obs_dim, n_op_actions, n_actions).to(device) for _ in range(self.n_rm_states)])
        for tgt, src in zip(self.q_targets, self.q_nets):
            tgt.load_state_dict(src.state_dict())

        # optimizers: one optimizer for opponent model, and one for all Q-nets
        self.op_optimizer = torch.optim.Adam(self.opponent_model.parameters(), lr=lr)
        # collect parameters from all q_nets
        q_params = []
    ...
    def select_action(self, obs: np.ndarray, rm_state: int, prev_op_action_idx: int, eps: float = 0.1):
        ...
        with torch.no_grad():
            ...
            q_vals = self.q_nets[rm_state](obs_t, op_pred_oh)  # use Q_net corresponding to current RM state
    ...
    def train_step(self):
        ...
        # targets[batch, u_cf] = r_cf + gamma * max_a' Q_target(s_next, u_cf_next, op_pred_next)
        ...
        with torch.no_grad():
            for u_cf in range(n_rm):
                ...
                # predict opponent in next state given u_cf_next (use observed op action as prev_op for next prediction)
                ...
                # compute q_target for each sample using the q_target network for u_cf_next
                ...
                for i in range(batch_size):
                    ...
                    q_vals_next = self.q_targets[u_next_i](s_next_batch[i:i+1], op_pred_next_oh[i:i+1])  # [1, n_actions]
        ...
        # Now compute losses for each u_cf: use q_net[u_cf] evaluated at (s_batch, op_pred_now based on u_cf)
        q_losses = []
        for u_cf in range(n_rm):
            ...
            q_vals = q_net(s_batch, op_pred_now_oh)  # [B, n_actions]
...
if __name__ == '__main__':
    ...
    rm = build_dummy_rm()
    agent = DQROMAgent(obs_dim, n_actions, rm, n_op_actions, prev_op_action_dim,
