...
class LowLevelAgent:
    ...
    def select_action(self, state, epsilon=0.9):  # fixed exploration
...
class HighLevelAgent:
    ...
    def select_option(self, state, epsilon=0.9):  # fixed exploration
        if random.random() < epsilon:
...
# ---------------------- DHRL Agent ----------------------
class DHRL:
    def __init__(self, env, state_dim, action_dim, num_options, option_length=10):
        self.env = env
        self.num_options = num_options
        self.option_length = option_length
        self.low_level_agents = [LowLevelAgent(state_dim, action_dim) for _ in range(num_options)]
        self.high_level_agent = HighLevelAgent(state_dim, num_options)
    ...
    def train(self, num_episodes=1000, epsilon=0.9, max_steps=200):
        for ep in range(num_episodes):
            ...
            while not done and step_count < max_steps:
                option = self.high_level_agent.select_option(state, epsilon)

                # Execute low-level policy for fixed option_length
                for _ in range(self.option_length):
                    action = self.low_level_agents[option].select_action(state, epsilon)
                    next_state, reward, done, _ = self.env.step(action)

                    # Store experiences
                    self.low_level_agents[option].replay_buffer.push(state, action, reward, next_state, done)
                    self.high_level_agent.replay_buffer.push(state, option, reward, next_state, done)
