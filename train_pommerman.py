...
def run(args):
    ...
    team = {aid: build_agent(args.algo, obs_dim=obs_dim, n_rm_states=n_rm_states, n_actions=N_ACTIONS, opponent_ids=opp_ids, n_opponent_actions=N_ACTIONS, device=device, seed=args.seed + i, buffer_size=args.buffer_size) for i, aid in enumerate(team_ids)}
    opp_rm = rms[opp_ids[0]]
    opp_u_to_index = {u: i for i, u in enumerate(opp_rm.states)}
    opp_n_rm_states = opp_rm.num_states
    opponents = {aid: DQNBaseline(obs_dim, opp_n_rm_states, N_ACTIONS, device=device, buffer_size=args.buffer_size, seed=args.seed + 100 + i) for i, aid in enumerate(opp_ids)}
    ...
    for ep in range(args.episodes):
        obs = sgrrm.reset()
        prev_opp_actions = {aid: 0 for aid in opp_ids}
        ep_reward = {aid: 0.0 for aid in sgrrm.agent_ids}
        for t in range(env.max_steps):
            actions = {}
            for aid in team_ids:
                s_vec, u = obs[aid]
                actions[aid] = team[aid].act(s_vec, u_to_index[u], epsilon)
            for aid in opp_ids:
                s_vec, u = obs[aid]
            ...
            for aid in team_ids:
                ...
                agent = team[aid]
                opp_actions = {o: actions[o] for o in opp_ids}
                agent.observe_opponents(s_vec, u_to_index[u], opp_actions, prev_opp_actions)
                remember_kwargs = {}
            ...
            prev_opp_actions = {aid: actions[aid] for aid in opp_ids}
